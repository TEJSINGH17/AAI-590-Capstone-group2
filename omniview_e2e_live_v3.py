...
## AR HUD System
 - Author : Sunitha Kosireddy
 - AAI 590 Capstone project, April 2026

## omniview_e2e_live_v3.ipynb — AR HUD System

This is the end-to-end runner for the OmniView AR HUD system. It orchestrates the full pipeline from setup to output across two operating modes.
The notebook contains 22 cells organized into 6 - **sequential stages:**
- **Setup Cell** — Clones the project repo from GitHub, installs dependencies (YOLOv8, OpenCV, ImageIO/FFmpeg), and verifies the source video file is present.
- **Cell #2 (Patch Cell)** — Applies 5 instrumentation patches to omniview_e2e_live.py, injecting performance counters for detections, blind spot alerts, fast-approach events, and per-frame render timing. Saves results to a JSON file after the pipeline runs.
-  **Mode 1 Cell (JSON File Mode)** — Runs the HUD pipeline in offline mode, reading pre-recorded detection data from JSON files and rendering the AR overlay onto the video.
- **DeepStream Cell** — Launches the live DeepStream dual-model pipeline (LISA + COCO) as a background process, patching it for the Colab environment and waiting for it to start before proceeding.
- **Mode 2 Cell (UDP Live Mode)** — Runs the HUD pipeline in live mode, receiving real-time detections from the DeepStream pipeline over UDP port 5055.
